...
from ..simulators import SVSimulator
from ..circuits.parameters import Parameter, ParameterExpression
import numpy as np
from ..exceptions import CircuitError
from ..circuits.gatelib import X_MAT, Y_MAT, Z_MAT, GATE_LIB
from ..circuits.gates import QuantumGate, CircuitWrapper, ControlledGate
...
def grad_para_shift(qc:QuantumCircuit, hamiltonian, backend=SVSimulator()):
    ...
    gate_grads= [[] for _ in qc.operations]
...
def grad_gate(op):
    ...
    if isinstance(op, ControlledGate):
        if op._targ_name == "RX":
            ...
            deriv_mat = -0.5j * X_MAT @ op._get_targ_matrix()
            ...
            circ = QuantumCircuit(max(op.pos)+1)
            deriv_mat = -0.5j * Y_MAT @ op._get_targ_matrix()
            ...
            circ = QuantumCircuit(max(op.pos)+1)
            deriv_mat = -0.5j * Z_MAT @ op._get_targ_matrix()
