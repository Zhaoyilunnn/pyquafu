...
A noise model is important when testing quantum error correction codes. The `qec` module will have a `noise_model.py` file. This file will let users add noise to their tests. The design of this noise model will be based on the [circuit-level noise model](https://quantumcomputing.stackexchange.com/a/39943/15231) from [Stim](https://github.com/quantumlib/Stim/blob/main/doc/getting_started.ipynb).

In this model, noise is added to all data qubits at certain points in the circuit. For example, a depolarizing channel can be added to all data qubits before each round of syndrome measurements, e.g., `before_round_data_depolarization`.
