...
class StateVector{
    ...
    public:
        ...
        //sample results
        std::unordered_map<std::string, int> measure_samples(std::map<int, int> meas, int shots);
...
#pragma omp parallel for
 ...
 //sample results
template <class real_t>
std::unordered_map<std::string, int> StateVector<real_t>::measure_samples(std::map<int, int> meas, int shots){
    std::unordered_map<std::string, int> counts;
    auto rands = Utils::randomDoubleArr(shots);
...
#pragma omp paraleel for reduction(+ : val)
        for (size_t j = 0;j < rsize;++j){
            uint z_phase_num =  Utils::popcount(j & z_mask) % 2;
...
#pragma omp parallel for reduction(+ : val)
        for (size_t j = 0;j < rsize;++j)
        {
            ...
            uint z_phase_num = Utils::popcount(i0 & z_mask) % 2;
            uint total_phase_num = z_phase_num * 2 + y_phase_num;
            std::complex<double> phase = Utils::PHASE_YZ[total_phase_num % 4];
